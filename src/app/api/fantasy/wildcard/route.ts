...
export async function POST(request: NextRequest) {
  try {
    ...
    // 3. Call the Postgres RPC to activate the wildcard
    const supabase = supabaseServer();
    const { data, error } = await supabase.rpc('activate_wildcard', {
      p_user_id: userId,
      p_fantasy_season_id: fantasySeasonId,
    });

    if (error) {
      console.error('Supabase RPC Error (activate_wildcard):', error);
      return NextResponse.json(
        { error: 'Database error occurred while activating wildcard.', details: error.message },
        { status: 500 }
      );
    }
    ...
    return NextResponse.json(
      { error: 'An unexpected error occurred while activating the wildcard.' },
      { status: 500 }
    );
  }
}
